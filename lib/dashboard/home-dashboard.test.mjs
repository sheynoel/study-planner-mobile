...
test('Today hero counts remaining classes and active tasks due on the local day', () => {
  ...
  assert.equal(hero.nextTask, null);
...
test('Today hero prefers a current timed event over the next class', () => {
  const event = classItem({ id: 'current-event', sourceType: 'event', startAt: '2026-08-07T10:15:00', endAt: '2026-08-07T11:00:00' });
  const nextClass = classItem({ id: 'next-class', startAt: '2026-08-07T12:00:00', endAt: '2026-08-07T13:00:00' });
  const hero = getHomeTodayHero([nextClass, event], [], now);
  assert.equal(hero.nextItem?.id, 'current-event');
  assert.equal(hero.nextState, 'current');
});

test('Today hero falls back to one important due item when no timed item remains', () => {
  const overdue = task({ id: 'overdue', dueAt: new Date(2026, 7, 6, 18).toISOString() });
  const todayTask = task({ id: 'today-task', dueAt: new Date(2026, 7, 7, 18).toISOString() });
  const hero = getHomeTodayHero([], [todayTask, overdue], now);
  assert.equal(hero.nextItem, null);
  assert.equal(hero.nextTask?.id, 'overdue');
  assert.equal(hero.nextState, 'due');
});
