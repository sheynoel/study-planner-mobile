...
import { filterHomeTasks, getClassReminders, getClassTimeState, getTodayClasses } from './home-dashboard.ts';
...
test('class reminders include at most two active same-course tasks due today', () => {
  const dueToday = (hours) => new Date(2026, 7, 7, hours).toISOString();
  const tasks = [
    task({ id: 'later', dueAt: dueToday(15) }),
    task({ id: 'first', dueAt: dueToday(9) }),
    task({ id: 'third', dueAt: dueToday(16) }),
    task({ id: 'other-course', courseId: 'course-2', dueAt: dueToday(10) }),
    task({ id: 'completed', status: 'COMPLETED', dueAt: dueToday(11) }),
  ];
  assert.deepEqual(getClassReminders(tasks, 'course-1', now).map((item) => item.id), ['first', 'later']);
});
