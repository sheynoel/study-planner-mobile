...
import { normalizeSemester, normalizeSemesters } from './semester-normalization.ts';
...
test('semester list, active state, and holidays use the validated canonical shape', () => {
  const semester = { id: 'semester-1', title: 'First Semester', startDate: '2026-08-01', endDate: '2026-12-20', isActive: true, holidays: [{ id: 'holiday-1', date: '2026-08-24', title: 'Holiday' }] };
  assert.deepEqual(normalizeSemester(semester), semester);
  assert.deepEqual(normalizeSemesters([semester]), [semester]);
  assert.equal(normalizeSemester({ ...semester, isActive: 'yes' }), null);
  assert.equal(normalizeSemester({ ...semester, holidays: [{ id: 'holiday-1', date: 'August 24', title: 'Holiday' }] }), null);
});
