...
export function FloatingQuickAdd() {
  ...
  return <><Pressable accessibilityLabel="Quick add" accessibilityRole="button" onPress={() => { setChoosingCourse(false); setVisible(true); }} style={({ pressed }) => [styles.button, { backgroundColor: colors.primary, bottom: 78 + insets.bottom }, pressed ? styles.pressed : undefined]}><Ionicons color={colors.primaryText} name="add" size={30} /></Pressable><BottomActionSheet actions={choosingCourse ? courseActions : baseActions} onClose={() => setVisible(false)} title={choosingCourse ? 'Choose a course' : 'Quick add'} visible={visible} /></>;
}

const styles = StyleSheet.create({ button: { alignItems: 'center', borderRadius: DesignTokens.radius.pill, height: 56, justifyContent: 'center', position: 'absolute', right: DesignTokens.layout.screenPadding, width: 56, zIndex: 30, ...Shadows }, pressed: { opacity: 0.82, transform: [{ scale: 0.96 }] } });
