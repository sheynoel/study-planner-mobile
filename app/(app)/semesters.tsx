...
import { activateSemester, addSemesterHoliday, createSemester, getSemesters, type Semester } from '@/lib/api/semesters';
...
export default function SemestersScreen() {
  ...
  async function activate(semester: Semester) { if (!accessToken) return; setError(null); try { await activateSemester(accessToken, semester.id); await refresh(); } catch (reason) { setError(getApiErrorMessage(reason)); } }
